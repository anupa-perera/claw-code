...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(report
            .artifacts
            .iter()
            .any(|artifact| artifact.name == ".claw/" && artifact.status == InitStatus::Created));
        assert!(report.artifacts.iter().any(
            |artifact| artifact.name == ".claw.json" && artifact.status == InitStatus::Created
        ));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        assert!(second
            .artifacts
            .iter()
            .any(|artifact| artifact.name == ".claw/" && artifact.status == InitStatus::Skipped));
        assert!(second.artifacts.iter().any(
            |artifact| artifact.name == ".claw.json" && artifact.status == InitStatus::Skipped
        ));
        assert_eq!(
